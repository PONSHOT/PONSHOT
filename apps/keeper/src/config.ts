...
function addressesFor(chainId: number): {prediction: Address; oracle: Address} {
  ...
  if (explicitPrediction && explicitOracle) return {prediction: explicitPrediction, oracle: explicitOracle};

  const path = process.env.DEPLOYMENTS_PATH ?? `${process.cwd()}/../../deployments/${chainId}.json`;
  try {
    const d = JSON.parse(readFileSync(path, "utf8"));
    return {prediction: d.PonsPrediction as Address, oracle: d.PonsOracleAdapter as Address};
  } catch (err) {
    throw new Error(
...
export function loadConfig(): KeeperConfig {
  ...
  const {prediction, oracle} = addressesFor(chainId);
