...
export async function POST(request: Request) {
  ...
  try {
    // Deliberately not `fetch`. The upstream sits behind Cloudflare, which challenges
    // Node's undici with a 403 interstitial while accepting `node:https` — see
    // packages/sdk/src/node-rpc.ts for the measurements. Long enough for a slow archival
    // read, short enough that a hung node does not pin a connection indefinitely.
    const result = await postJsonRpc(UPSTREAM(), body, {timeoutMs: 20_000});
