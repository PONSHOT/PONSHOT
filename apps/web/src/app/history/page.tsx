...
import {changeBps, formatChangeBps, formatEth, formatWethPerPons, describeOutcome} from "@pons/sdk";
...
const OUTCOME_TONE: Record<string, string> = {
  BULL: "bg-up-500/15 text-up-400",
  BEAR: "bg-down-500/15 text-down-400",
  TIE: "bg-base-700 text-mute-400",
  NO_CONTEST: "bg-base-700 text-mute-400",
  ALL_LOST: "bg-burn-500/15 text-burn-400",
  CANCELLED: "bg-down-500/15 text-down-400",
};

/** Labels come from the SDK so the API and the UI cannot describe a round differently. */
function OutcomeBadge({value, status, reason}: {value: string | null; status: string; reason: string | null}) {
  const key = status === "CANCELLED" ? "CANCELLED" : (value ?? "");
  const {label, detail} = describeOutcome(key);
  const tone = OUTCOME_TONE[key] ?? "bg-base-700 text-mute-500";
  return (
    <span className={`rounded px-2 py-0.5 text-[11px] font-semibold ${tone}`} title={reason ?? detail}>
      {label}
    </span>
  );
}
