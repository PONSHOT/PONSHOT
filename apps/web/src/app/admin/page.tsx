...
import {MarketAbiWithLegacy, UniswapV3PonsOracleAbi, formatEth} from "@pons/sdk";
...
export default function AdminPage() {
  ...
  const {data: state} = useReadContracts({
    contracts: [
      ...
      {...base, functionName: "burnAllocated"},
      ...
      // Legacy names, for a market deployed before the fee was renamed.
  ...
  const burnPending = (state?.[2]?.result as bigint | undefined) ?? (state?.[11]?.result as bigint | undefined) ?? 0n;
  ...
  // No `burnAllocated` means a market that predates buyback-and-burn.
  const legacyMarket = state?.[2]?.result === undefined;
  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat label="Burn allocation (unswept)" value={`${formatEth(burnPending)} ETH`} />
...
function RoleControls({
  prediction, isAdmin, isPauser, isConfig, paused, burnPending, legacyMarket,
}: {
  prediction: `0x${string}`; isAdmin?: boolean; isPauser?: boolean; isConfig?: boolean;
  paused: boolean; burnPending: bigint; legacyMarket: boolean;
}) {
  ...
  const call = (functionName: string, args: readonly unknown[]) =>
    writeContract({address: prediction, abi: MarketAbiWithLegacy, functionName: functionName as never, args: args as never});
          ...
          {burnPending > 0n && (
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                disabled={isPending}
                onClick={() =>
                  // v2 sweeps the whole allocation and takes no argument; v1 withdraws a
                  // named amount to the treasury. Which one exists is what the reads above
                  // already told us.
                  legacyMarket ? call("claimTreasury", [burnPending]) : call("sweepToBurner", [])
                }
              >
                {legacyMarket ? `Withdraw ${formatEth(burnPending)} ETH of fees` : `Sweep ${formatEth(burnPending)} ETH to the burner`}
              </button>
