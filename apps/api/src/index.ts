...
import {MarketAbiWithLegacy, PonsBuybackBurnerAbi, UniswapV3PonsOracleAbi, readFeeBooked, readFeeBps} from "@pons/sdk";
...
const BURNER = (deployment.PonsBuybackBurner ?? null) as Address | null;
...
const client = createPublicClient({chain, transport: fallback(rpcUrls.map((u) => nodeHttp(u, {timeoutMs: 15_000})))});

const readMarket = (fn: string, args: readonly unknown[] = []) =>
  client.readContract({address: PREDICTION, abi: MarketAbiWithLegacy, functionName: fn as never, args: args as never});
...
const readBurner = (fn: string, args: readonly unknown[] = []) =>
  client.readContract({
    address: BURNER as Address,
...
router.get("/prediction/stats", async () => {
  const {rows} = await db.query<Record<string, string>>(
    `SELECT
       ...
       (SELECT COALESCE(SUM(burn_fee),0) FROM rounds WHERE chain_id=$1)             AS burn_fees_accrued,
       ...
       (SELECT COUNT(*) FROM rounds WHERE chain_id=$1 AND outcome='ALL_LOST')           AS all_lost_rounds,
       (SELECT COALESCE(SUM(burn_fee),0) FROM rounds WHERE chain_id=$1 AND outcome='ALL_LOST')
                                                                                       AS all_lost_burned`,
    [CHAIN_ID]
  );
...
/**
 * Buyback and burn: what has been destroyed, and what is queued to be.
 *
 * Burned totals come from indexed `BoughtAndBurned` events rather than from a running
 * total held anywhere, so the figure is a sum over things that provably happened. The
 * pending figures are read live, because they change every time a round settles.
 */
router.get("/burn", async () => {
  if (!BURNER) {
    return {
      configured: false,
      note: "this deployment predates buyback-and-burn; the protocol fee accrued to a treasury",
    };
  }

  const rows = await db.query(
    `SELECT target_index, token, COUNT(*) AS burns,
            COALESCE(SUM(eth_in),0)  AS eth_spent,
            COALESCE(SUM(burned),0)  AS tokens_burned,
            MAX(block_time)          AS last_burn_at
     FROM burns WHERE chain_id=$1 GROUP BY target_index, token ORDER BY target_index`,
    [CHAIN_ID]
  );

  const [pendingInMarket, allocatedPons, allocatedProject, targetPons, targetProject] = await Promise.all([
    readFeeBooked(readMarket),
    readBurner("allocated", [0n]),
    readBurner("allocated", [1n]),
    readBurner("target", [0]),
    readBurner("target", [1]),
  ]);

  const named = ["PONS", "PROJECT"] as const;
  return {
    configured: true,
    burner: BURNER,
    /** Booked by settled rounds, not yet pushed to the burner. Anyone may sweep it. */
    pendingInMarket: String(pendingInMarket),
    targets: named.map((name, i) => {
      const t = (i === 0 ? targetPons : targetProject) as {token: Address; pool: Address; shareBps: number};
      const row = rows.rows.find((r) => Number(r.target_index) === i);
      return {
        name,
        index: i,
        token: t.token,
        pool: t.pool,
        shareBps: Number(t.shareBps),
        configured: t.token !== "0x0000000000000000000000000000000000000000",
        allocated: String(i === 0 ? allocatedPons : allocatedProject),
        burns: Number(row?.burns ?? 0),
        ethSpent: String(row?.eth_spent ?? "0"),
        tokensBurned: String(row?.tokens_burned ?? "0"),
        lastBurnAt: row?.last_burn_at ?? null,
      };
    }),
  };
});
