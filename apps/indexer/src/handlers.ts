...
const OUTCOME = ["UNDECIDED", "BULL", "BEAR", "TIE", "NO_CONTEST"] as const;
...
export async function applyLog(client: PoolClient, chainId: number, ev: DecodedLog): Promise<void> {
  ...
  switch (ev.name) {
    ...
    case "RoundSettled": {
      ...
      await client.query(
        `UPDATE rounds SET close_price=$3, close_tick=$4, settled_at=$5, status='SETTLED',
                           outcome=$6, reward_base=$7, reward_amount=$8, treasury_fee=$9
         WHERE chain_id=$1 AND epoch=$2`,
        [
          chainId, epoch, n(ev.args.closePrice), Number(ev.args.closeTick ?? 0), executedAt.toString(),
          outcome, n(ev.args.rewardBaseAmount), n(ev.args.rewardAmount), n(ev.args.treasuryFee),
        ]
      );
      if (instant > 0n) {
        await client.query(
      ...
      if (BigInt(n(ev.args.treasuryFee)) > 0n) {
        await client.query(
          `INSERT INTO treasury_events (chain_id, tx_hash, log_index, block_number, kind, amount, detail, block_time)
           VALUES ($1,$2,$3,$4,'FEE_ACCRUED',$5,$6,$7)
           ON CONFLICT (chain_id, tx_hash, log_index) DO NOTHING`,
          [chainId, ev.transactionHash, ev.logIndex, ev.blockNumber.toString(), n(ev.args.treasuryFee),
    ...
    case "TreasuryClaim":
      await client.query(
        `INSERT INTO treasury_events (chain_id, tx_hash, log_index, block_number, kind, amount, detail, block_time)
         VALUES ($1,$2,$3,$4,'FEE_WITHDRAWN',$5,$6,$7)
         ON CONFLICT (chain_id, tx_hash, log_index) DO NOTHING`,
...
export async function rollbackFrom(db: Db, chainId: number, fromBlock: bigint): Promise<void> {
  ...
  try {
    ...
    await client.query(`DELETE FROM treasury_events WHERE chain_id=$1 AND block_number >= $2`, [chainId, b]);
