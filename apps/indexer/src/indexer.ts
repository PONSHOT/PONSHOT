...
export class Indexer {
  ...
  constructor(
    ...
    this.client = createPublicClient({
      chain,
      transport: fallback(
        cfg.rpcUrls.map((u) => http(u, {timeout: 20_000, retryCount: 2})),
        {rank: false}
      ),
    });
  }
  ...
  private async indexRange(from: bigint, to: bigint): Promise<void> {
    const raw = await this.client.getLogs({address: this.cfg.prediction, fromBlock: from, toBlock: to});
    const parsed = parseEventLogs({abi: PonsPredictionAbi, logs: raw, strict: false});
  ...
  private async wipeChain(): Promise<void> {
    ...
    try {
      await client.query("BEGIN");
      for (const table of [
        "events", "bets", "claims", "keeper_executions", "treasury_events",
