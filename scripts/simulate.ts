 ...
 * burn accounting is checked at every step.
 *
...
async function main() {
  ...
  const feeBps = Number(await read("burnFeeBps"));
  ...
  const burnBooked = (await read("burnAllocated")) as bigint;
  assert(burnBooked === (settled.totalAmount * BigInt(feeBps)) / 10_000n, "burn allocation wrong");
  ok(`burn allocation booked ${formatEth(burnBooked)} ETH (${feeBps / 100}%)`);
  ...
  step(11, "Solvency and burn accounting");
  ...
  // The sweep is permissionless and takes no amount: it moves the whole booked
  // allocation. Sent from a non-operator account on purpose, because "anyone can push
  // this" is a property worth failing on if it ever stops being true.
  const burnerAddr = (await read("burner")) as `0x${string}`;
  const burnerBefore = await pub.getBalance({address: burnerAddr});
  await send(bob, "sweepToBurner", []);
  const burnerAfter = await pub.getBalance({address: burnerAddr});
  assert(burnerAfter - burnerBefore === burnBooked, "burn allocation did not reach the burner");
  assert(((await read("burnAllocated")) as bigint) === 0n, "burn allocation not cleared");
  ok(`swept ${formatEth(burnBooked)} ETH to the burner (sent by a non-operator)`);

  const [, owed2, solvent2] = (await read("solvency")) as [bigint, bigint, boolean];
  assert(solvent2, "insolvent after the burn sweep");
