...
# The burner is deployed by the script; only the project token needs supplying.
...
# DEFAULT_ADMIN_ROLE and can change where the burn allocation is sent.
...
( cd "$ROOT/packages/contracts" \
  && DEPLOYER_PRIVATE_KEY="$DEPLOYER_PRIVATE_KEY" \
     ADMIN_ADDRESS="$ADMIN" OPERATOR_ADDRESS="$KEEPER_ADDR" \
     PROJECT_TOKEN="$PROJECT_TOKEN" PROJECT_POOL="$PROJECT_POOL" \
     ROUND_INTERVAL="${ROUND_INTERVAL:-900}" TWAP_WINDOW="${TWAP_WINDOW:-300}" \
     BUFFER_SECONDS="${BUFFER_SECONDS:-1800}" BURN_FEE_BPS="${BURN_FEE_BPS:-1000}" \
