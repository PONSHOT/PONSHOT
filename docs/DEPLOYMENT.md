...
## Proving the deployed code is this source

Explorer verification is the presentation of this fact; it is not the fact. The fact is
checkable directly:
...
npx tsx scripts/verify-bytecode.mts PonsPrediction 0xC463621052E57Cfa2F8CDA86ee337d8d9aD4dBD0 \
  --out /tmp/v1/packages/contracts/out
```

Result for the live deployment, at tag `v1.0.0`:

| Contract | Result |
|---|---|
| `PonsPrediction` | 18,487 bytes, **byte-for-byte identical** |
| `CompositePonsOracle` | 10,656 bytes, identical apart from 6 immutables at 14 positions |

An `immutable` is written into the runtime code at construction, so a fresh compile has
zeros where the deployed copy holds real values. That is not a mismatch, and the script
masks those ranges rather than reporting one — but it means the values themselves need a
separate check, by reading the getters:

```
baseToken         0x39dBED3a2bd333467115dE45665cC57F813C4571   (PONS)
quoteToken        0x0Bd7D308f8E1639FAb988df18A8011f41EAcAD73   (WETH)
baseUnit          1000000000000000000
defaultTwapWindow 300
...
The build settings must match the deployment exactly — solc 0.8.24, optimizer on at 1000
runs, `bytecode_hash = "none"`, `cbor_metadata = false`, evm version cancun. They are
pinned in `foundry.toml`. If a comparison fails, do not adjust them to make it pass: that
verifies different bytecode.

**Blockscout's verification API cannot be reached from a server.** It sits behind
Cloudflare, which challenges non-browser clients, so `forge verify-contract` returns a
"Just a moment..." interstitial instead of JSON. Verification has to go through the
explorer's web UI by hand. The bytecode check above does not depend on it.
