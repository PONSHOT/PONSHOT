    ...
    function invariant_terminalStatesAreConsistent() public view {
        for (uint256 e = 1; e <= market.currentEpoch(); e++) {
            ...
            if (r.status == RoundStatus.Settled) {
                ...
                // The outcome must be exactly what the two prices imply.
                if (r.bullAmount == 0 || r.bearAmount == 0) {
                    assertTrue(t.outcome == Outcome.NoContest, "one-sided round was not a no-contest");
                } else if (r.closePrice > r.lockPrice) {
                    assertTrue(t.outcome == Outcome.Bull, "outcome contradicts the prices");
                } else if (r.closePrice < r.lockPrice) {
                    assertTrue(t.outcome == Outcome.Bear, "outcome contradicts the prices");
                } else {
