...
contract PonsPredictionPermissionsTest is PredictionHarness {
    ...
    function test_operatorRoleGrantsNoEconomicPower() public {
        ...
        vm.expectRevert();
        market.claimTreasury(1);
        vm.expectRevert();
        market.setTreasury(operator);
        vm.expectRevert();
        market.proposeTreasuryFee(500);
        vm.expectRevert();
    ...
    function test_feeIsCappedByTheContractItself() public {
        vm.prank(admin);
        vm.expectRevert(abi.encodeWithSelector(PonsPrediction.FeeTooHigh.selector, uint256(501), uint256(500)));
        market.proposeTreasuryFee(501);
        assertEq(market.MAX_TREASURY_FEE_BPS(), 500);
    }

    function test_feeChangeIsTimelocked() public {
        vm.prank(admin);
        market.proposeTreasuryFee(400);
        ...
        vm.prank(admin);
        vm.expectRevert();
        market.commitTreasuryFee();
        assertEq(market.treasuryFeeBps(), FEE_BPS, "fee changed before the delay elapsed");

        vm.warp(block.timestamp + market.CONFIG_TIMELOCK());
        vm.prank(admin);
        market.commitTreasuryFee();
        assertEq(market.treasuryFeeBps(), 400);
    }
    ...
    function test_configChangesCannotReachAnAlreadyOpenRound() public {
        ...
        assertEq(before.treasuryFeeBps, FEE_BPS);
        ...
        assertEq(afterT.treasuryFeeBps, FEE_BPS, "open round's fee was rewritten");
