...
/// `sweepToBurner` is bounded by `burnAllocated`, which only ever grows by fees
...
contract PonsPrediction is IPonsPredictionTypes, AccessControl, Pausable, ReentrancyGuard {
    ...
    /// @dev 10% of the pooled stake, matching the launch tokenomics. The ceiling is a
    ///      constant rather than a config value so that the contract itself, rather than
    ///      governance, is the guarantee bettors rely on: whatever else changes, a
    ///      contested round always pays at least 90% of the pot back out to winners.
    uint256 public constant MAX_BURN_FEE_BPS = 1000;
    ...
    error BurnSweepOverdraw(uint256 requested, uint256 available);
    ...
    event BurnSwept(address indexed to, uint256 amount);
    ...
    event BurnerUpdated(address previous, address next);
    ...
    uint256 public burnAllocated;

    /// @notice ETH this contract owes to users: live stakes, unclaimed rewards, unclaimed refunds.
    /// @dev The core solvency invariant is `address(this).balance >= totalLiabilities + burnAllocated`.
    ...
    constructor(InitParams memory p) {
        if (p.admin == address(0) || p.burner == address(0) || p.oracle == address(0)) revert ZeroAddress();
        ...
        if (p.burnFeeBps > MAX_BURN_FEE_BPS) revert FeeTooHigh(p.burnFeeBps, MAX_BURN_FEE_BPS);
    ...
    function _trySettle(uint256 epoch) private returns (bool) {
        ...
        if (r.totalAmount == 0) {
            ...
            // The price did not move, so neither side was right. Refund.
            outcome = Outcome.Tie;
        } else {
            bool bullWins = price > r.lockPrice;
            winningPool = bullWins ? r.bullAmount : r.bearAmount;
            uint256 losingPool = bullWins ? r.bearAmount : r.bullAmount;

            if (winningPool == 0) {
                // Every entry was on the losing side. There is nobody to pay, so the
                // whole pot funds the buyback and burn rather than sitting here forever.
                outcome = Outcome.AllLost;
                fee = r.totalAmount;
            } else if (losingPool == 0) {
                // Everyone was right and nobody took the other side, so nothing was won.
                // Charging a rake on a bettor's own stake here would be a pure loss for
                // being right, so the round refunds in full.
                outcome = Outcome.NoContest;
                winningPool = 0;
            } else {
                outcome = bullWins ? Outcome.Bull : Outcome.Bear;
                fee = (r.totalAmount * t.burnFeeBps) / MAX_BPS;
    ...
    /// @notice Sends the booked burn allocation to the burner, which buys and burns.
    ///
    /// @dev Permissionless, for the same reason the lifecycle is: it cannot be aimed
    ///      anywhere but the configured burner, so there is nothing worth gating. The
    ///      keeper calls it immediately after settling a round; if the keeper is dead,
    ///      anyone can, and the funds are not stuck behind an operator.
    ///
    ///      This is deliberately *not* done inside `_trySettle`. Settlement must never
    ///      contain an external call that can revert, or a dry pool could stall a market
    ///      holding user stakes. It must also never trade in the pools the oracle reads,
    ///      or the market would be moving the price its own next round settles against.
    ///      `PonsBuybackBurner` carries the full argument.
    ///
    ///      Bounded by `burnAllocated`, which grows only from fees booked at settlement,
    ///      so this can never reach ETH owed to users.
    function sweepToBurner() external nonReentrant returns (uint256 amount) {
        amount = burnAllocated;
        if (amount == 0) revert BurnSweepOverdraw(0, 0);
        burnAllocated = 0;
        emit BurnSwept(burner, amount);
        _send(burner, amount);
    }
    ...
    function proposeBurnFee(uint32 bps) external onlyRole(CONFIG_ROLE) {
        if (bps > MAX_BURN_FEE_BPS) revert FeeTooHigh(bps, MAX_BURN_FEE_BPS);
        _pendingFeeBps = bps;
        _pendingFeeReadyAt = block.timestamp + CONFIG_TIMELOCK;
        emit FeeChangeProposed(burnFeeBps, bps, _pendingFeeReadyAt);
    }

    function commitBurnFee() external onlyRole(CONFIG_ROLE) {
        ...
        uint32 previous = burnFeeBps;
        burnFeeBps = _pendingFeeBps;
        _pendingFeeReadyAt = 0;
        emit FeeUpdated(previous, burnFeeBps);
    }
    ...
    function setBurner(address next) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (next == address(0)) revert ZeroAddress();
        emit BurnerUpdated(burner, next);
        burner = next;
    }
    ...
    function solvency() external view returns (uint256 balance, uint256 owed, bool solvent) {
        ...
        owed = totalLiabilities + burnAllocated;
