...
export function treasuryFee(totalAmount: bigint, feeBps: number | bigint): bigint {
 ...
 *
 * Returns 0 for a round that cannot pay out — one where a side attracted no stake. The
 * contract treats those as `NoContest` and refunds in full rather than raking a bettor's
 * own money, so a UI that showed a multiplier here would be promising winnings that
 * cannot exist.
 */
export function rewardPool(bullAmount: bigint, bearAmount: bigint, feeBps: number | bigint): bigint {
  ...
  return total - treasuryFee(total, feeBps);
}
